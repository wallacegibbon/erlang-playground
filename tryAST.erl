...
json([?FUNCTION(Clauses) | Rest], Parsed)   -> json(Rest, [?FUNCTION((json_clauses(Clauses))) | Parsed]);
json([Other | Rest], Parsed)                -> json(Rest, [Other | Parsed]);
json([], Parsed)                            -> lists:reverse(Parsed).

json_clauses([{clause, Line, A1, A2, Code} | Clauses])  -> [{clause, Line, A1, A2, json_code(Code)} | json_clauses(Clauses)];
json_clauses([])                                        -> [].

-define(JSON(Json), {bin, _, [{bin_element, _, {tuple, _, [Json]}, _, _}]}).
...
%% string, number
parse_json({string, Line, String})                                                  -> {string, Line, String};
parse_json({integer, Line, Integer})                                                -> {integer, Line, Integer};
parse_json({float, Line, Float})                                                    -> {float, Line, Float};
parse_json({op, Line, '-', {Type, _, N}}) when Type =:= integer; Type =:= float     -> {Type, Line, -N};
%% true, false, null
...
parse_json_fields([{remote, L1, Key, Value} | Rest], Line)  -> {cons, L1, {tuple, L1, [parse_json(Key), parse_json(Value)]}, parse_json_fields(Rest, Line)};
