...
init() ->
    ...
    ok = mnesia:start(),
    {atomic, ok} = mnesia:create_table(shop,
                                       [{attributes,
                                         record_info(fields, shop)}]),
    {atomic, ok} = mnesia:create_table(blah,
                                       [{attributes,
                                         record_info(fields, blah)}]),
    mnesia:dirty_write(#shop{item = apple, quantity = 10,
                             cost = 20}),
...
test_read_sub() ->
    [Inc] = qlc:e(qlc:q([Inc
                         || #blah{item = apple, value = Inc}
                                <- mnesia:table(blah)])),
    [R1] = qlc:e(qlc:q([X || X <- mnesia:table(shop)])),
    fmt("[~w] qlc in, ~p~n", [self(), R1]),
    sleep(1000),
    mnesia:write(#shop{item = apple, quantity = 10,
                       cost = R1#shop.cost + Inc}),
...
fmt(Fmt, Args) ->
    ...
    T = calendar:system_time_to_rfc3339(Now,
                                        [{unit, nanosecond}]),
