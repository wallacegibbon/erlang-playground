...
read(Filename) ->
    ...
    <<"FOR1", Size:32/integer, "BEAM", Chunks/binary>> =
        Content,
...
read_chunks(<<N, A, M, E, Size:32/integer,
              Tail/binary>>,
            Acc) ->
...
parse_chunks([{"ExpT",
             ...
             Acc) ->
    parse_chunks(Rest,
                 [{exports, parse_exports(Exports)} | Acc]);
parse_chunks([{"Code",
               Size,
               <<Subsize:32/integer, Chunk/binary>>}
              | Rest],
             Acc) ->
    ...
    parse_chunks(Rest,
                 [{code, parse_code_info(Info), Opcodes} | Acc]);
parse_chunks([{"StrT", _Size, <<Strings/binary>>}
              | Rest],
             Acc) ->
    parse_chunks(Rest,
                 [{string, binary_to_list(Strings)} | Acc]);
...
parse_atoms(<<Atomlength, Atom:Atomlength/binary,
              Rest/binary>>)
    when Atomlength > 0 ->
...
parse_exports(<<Function:32/integer, Arity:32/integer,
                Label:32/integer, Rest/binary>>) ->
    [{Function, Arity, Label} | parse_exports(Rest)];
parse_exports(_Alignment) -> [].

parse_code_info(<<Instructionset:32/integer,
                  OpcodeMax:32/integer, NumberOfLabels:32/integer,
...
parse_literals(<<Size:32, Literal:Size/binary,
                 Tail/binary>>) ->
